...
export default function ShinchoSummaryPanel() {
  ...
  const loadData = useCallback(() => {
    ...
    setLoading(true);
    window.eel.fetch_table_rows('shincho_data')((res: any) => {
      setLoading(false);
    ...
    <div className="bg-white border border-slate-300 rounded-lg shadow-sm flex flex-col h-full min-h-[600px] overflow-hidden">
      ...
      <div className="flex justify-between items-center px-4 py-2 bg-[#ffe699] text-slate-800">
        <div className="flex items-center space-x-2">
          ...
          <h2 className="font-bold text-sm">03-02 新朝プレス 発注集計</h2>
        </div>
        <div className="flex items-center space-x-2">
          <Settings size={16} className="cursor-pointer hover:rotate-90 transition-transform opacity-80" />
          <RefreshCcw
            size={16}
            className={`cursor-pointer hover:rotate-180 transition-transform opacity-80 ${loading ? 'animate-spin' : ''}`}
            onClick={loadData}
          />
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col text-xs text-slate-800">
        ...
        <div className="overflow-y-auto flex-1">
          ...
          {!loading && items.map((item, idx) => (
            <div key={idx} className="flex justify-between items-center px-3 py-2 border-b border-slate-100 hover:bg-amber-50 transition-colors">
              <span className="truncate pr-4">{item.name}</span>
              <span className="text-amber-600 font-bold whitespace-nowrap">{item.count}ケース</span>
            </div>
          ))}
        </div>
      </div>
